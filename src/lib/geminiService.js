...
// Helper function to call our secure backend API endpoint
async function callApiProxy(text, prompt) {
  try {
    // This fetch request goes to our own backend function, not directly to Google
...
// These functions use the secure proxy helper
export function getStructuredDataFromGemini(text) {
  return callApiProxy(text, STRUCTURE_SYSTEM_PROMPT);
}

export function getInsightsFromGemini(text) {
  return callApiProxy(text, INSIGHTS_SYSTEM_PROMPT);
}